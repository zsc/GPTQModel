...
Optimized version for MoE models.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--configs",
        default="bf16,4,6,8",
        help="Comma-separated list of bit widths to test (e.g., 'bf16,4,6,8')",
    )
    parser.add_argument(
        "--outlier-percentile",
        type=float,
...
def main() -> None:
    ...
    for bits in config_bits:
        if bits is None:
            configs.append(ExperimentConfig("BF16 baseline", None, args.outlier_percentile))
        else:
