#!/usr/bin/env python3
"""
Lightweight quantization experiments for Qwen3-30B-A3B-Instruct-2507.
Optimized for large MoE models with limited GPU memory.
"""
    ...
    *,
    ...
    for begin_loc in range(0, seq_len, stride):
        ...
        if loss is not None:
            ...
            if logits is None:
                raise RuntimeError("Model output has neither `.loss` nor `.logits`")
            logits = logits.float()
...
def _load_model_and_tokenizer(model_path: str, *, use_fast: Optional[bool], tp_size: int = 2) -> Tuple[Any, Any]:
    import deepspeed
    ...
    print(f"  Initializing DeepSpeed with TP={tp_size}...")
    init_distributed()
    ...
    print("  Loading model with DeepSpeed TP...")
    model = AutoModelForCausalLM.from_pretrained(
        model_path, 
        **model_kwargs
    )
    
    # Create DeepSpeed config for TP
    ds_config = {
        "tensor_parallel": {
            "tp_size": tp_size,
        },
    ...
    model = deepspeed.init_inference(
        model,
        tensor_parallel=ds_config["tensor_parallel"],
        dtype=torch.bfloat16,
    )
    print(f"  Model loaded successfully with DeepSpeed TP={tp_size}")
...
def main():
    ...
    parser.add_argument("--results-dir", type=Path, default=Path("results/qwen3_30b_a3b"))
    ...
    parser.add_argument("--quantile-sample-size", type=int, default=100_000)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--act-calib-chars", type=int, default=10000)
    parser.add_argument("--act-calib-seq-len", type=int, default=256)
    parser.add_argument("--skip-existing", action="store_true")
    parser.add_argument("--include-wa", action="store_true")
    parser.add_argument(
        "--model-path",
        default="/publicdata/huggingface.co/Qwen/Qwen3-30B-A3B-Instruct-2507",
    )
    parser.add_argument("--config", required=True, help="Config to run: bf16, 4, 6, 8, 6wa, 8wa")
    ...
    if cfg == "bf16" or cfg == "baseline":
        config = ExperimentConfig("BF16 baseline", None, 1.0)
        out_name = f"{model_key}_bf16.json"
    elif cfg in ["4", "6", "8"]:
        bits = int(cfg)
        config = ExperimentConfig(f"{bits}-bit W (1.0%)", bits, 1.0)
        out_name = f"{model_key}_{bits}bit_outlier1.0.json"
    elif cfg == "6wa":
        config = ExperimentConfig("6-bit W+A (1.0%)", 6, 1.0, act_bits=6, act_outlier_percentile=1.0)
        out_name = f"{model_key}_6bit_outlier1.0_act6bit_actoutlier1.0.json"
    elif cfg == "8wa":
        config = ExperimentConfig("8-bit W+A (1.0%)", 8, 1.0, act_bits=8, act_outlier_percentile=1.0)
        out_name = f"{model_key}_8bit_outlier1.0_act8bit_actoutlier1.0.json"
    else:
